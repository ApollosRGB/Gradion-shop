...
![status](https://img.shields.io/badge/version-1.10.7-e0563f)
...
- **Setup sync (GitHub)** — installing on another machine used to mean typing the whole configuration again, so the shop's setup lives in **one JSON file in a GitHub repository**. **Publish this setup** writes products, stations, robots, nodes, robot↔station access, recalls (with their automation settings) and the SYNAOS / arm / MPDV connection details to it; **Load setup from GitHub** pulls them onto another machine, offered on the first-run screen as well as in admin so a fresh install never needs the admin password just to be configured. What stays behind is what belongs to the machine: its orders, logs, MPDV counter, in-flight hand-overs, each recall's live countdown, and its theme.
...
- **Windows** — `GradionShop-Setup-1.10.7.exe`
- **macOS** — `GradionShop-1.10.7.dmg`
